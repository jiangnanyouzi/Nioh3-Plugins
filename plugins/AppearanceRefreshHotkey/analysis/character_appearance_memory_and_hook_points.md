...
| Indexed state-word writer | `0x22390B8` | `0x2176BB8` | Identical body (`cmp edx,27` / `mov [rcx+rax*2],r8w`). |
| RefreshPlayerAppearance | `0x2477990` | `0x235092C` | Called immediately after the bridge at both UI call sites `+0x2341CD5` / `+0x23422F2`. Its prologue pattern has byte-identical sibling clones; the source signature spans the whole body plus the next function's prologue to stay unique. |
