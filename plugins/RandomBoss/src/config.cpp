...
void LoadConfig(const Nioh3PluginInitializeParam* param) {
  ...
  g_mapIgnoreBlocked.store(std::strtoul(value, nullptr, 0) != 0,
                           std::memory_order_release);
  // MapPurpleRender was read here. REMOVED 2026-09-22: the render gate it drove
  // was falsified by isolation (see the tombstone in patterns.h), so the key is
  // gone rather than left as a switch that does nothing.
